...
class Mienviorates extends AbstractCarrier implements CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        /**/
        /*
         * buscar parametro filter list
         * */
        $useFilterList = false;

        $this->_logger->debug('USE FILTER LIST', ['use_filter_list' => $shippingAddress->getData('customer_id')]);
        $this->_logger->debug('USE FILTER LIST', ['use_filter_list' => $shippingAddress->getData()]);
        $customer = $this->customerFactory->create();
        $customerModel = $customer->load($shippingAddress->getData('customer_id'));

        if ($customerModel->getAddresses() != null) {
            foreach ($customerModel->getAddresses() as $address) {
                $customerAddressData = $address->toArray();
                $this->_logger->debug('USE FILTER LIST', ['street - 1' => $customerAddressData['street']]);
                $this->_logger->debug('USE FILTER LIST', ['street - 2' => $request->getDestStreet()]);
                if ($customerAddressData['street'] === $request->getDestStreet()) {
                    if ($customerAddressData['filter_list'] == 1) {
                        $useFilterList = true;
                    } else {
                        $useFilterList = false;
                    }
                ...
                $this->_logger->debug('USE FILTER LIST', ['use_filter_list' => $address->toArray()]);
            }
        }
        $this->_logger->debug('USE FILTER LIST', ['$useFilterList' => $useFilterList]);
        /**/
        ...
        $filterList = null;
        if ($useFilterList) {
            $filterList = trim($this->_scopeConfig->getValue(
                'filter_data/filter_data_group/filter_data_group_field', \Magento\Store\Model\ScopeInterface::SCOPE_WEBSITES, $this->_storeManager->getWebsite()->getId()
            ));

            /* $this->_logger->debug('USE FILTER LIST', ['$useFilterList' => \Magento\Store\Model\ScopeInterface::SCOPE_WEBSITES]);
             $this->_logger->debug('USE FILTER LIST', ['$useFilterList' => $this->_storeManager->getWebsite()->getId()]);*/
        }
        $this->_logger->debug('USE FILTER LIST', ['$useFilterList' => $filterList]);
