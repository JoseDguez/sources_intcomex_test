<?php
namespace Esguerra\Freeproduct\Plugin;
...
class Related extends RelatedParent {
    ...
    protected function getCustomlinkedFieldset($modify) {
        $content = __(
            'Intcomex free prodcuts. Products with price Zero (0).'
        );
        return [
            'children' => [
                'button_set' => $modify->getButtonSet(
                    $content, __('Add Free Product'), $modify->scopePrefix . static::DATA_SCOPE_CUSTOMLINKED
                ),
                'modal' => $this->getGenericModal(
                    __('Add Free Product'), $modify->scopePrefix . static::DATA_SCOPE_CUSTOMLINKED
                ),
    ...
    public function beforeGetLinkedProducts($provider, $product) {
        ...
        $this->product = $objectManager->create('Esguerra\Freeproduct\Model\Product');
